...
export default function BarDisplay() {
  ...
  const [preparingItems, setPreparingItems] = useState(new Set())
  ...
  const loadOrders = async () => {
    try {
      ...
      const list = filterBarOrders(res.orders || [])
      setOrders(list)
      // Seed timers for items already in preparing state (default estimate on page load)
      setItemTimers(prev => {
        const next = { ...prev }
        list.forEach(o => {
          o.items.forEach(i => {
  ...
  // Load initial low/out bar ingredients for the banner
  useEffect(() => {
    const load = async () => {
      try {
        const [r1, r2] = await Promise.all([
          fetchIngredients({ destination: "bar",  limit: 200 }),
  ...
  useSocket(
    {
      "new-order": () => {
        ...
        loadOrders().then(() => {
          setTimeout(() => {
            setOrders(current => {
              const cards = getItemCards(filterBarOrders(current))
              const newKeys = cards
      ...
      "item-status-update": ({ orderId, itemId, status, orderStatus }) => {
        ...
        // Clean up the timer when this item is no longer preparing
    ...
    [{ event: "join-bar", value: null }]
  )

  // ── actions ────────────────────────────────────────────────────────────────

  const handleStartPreparing = async (card, estimatedSecs) => {
    const key = `${card.order._id}-${card._id}`
    setPreparingItems(prev => new Set(prev).add(key))
    ...
    try {
      ...
      setPreparingItems(prev => { const s = new Set(prev); s.delete(key); return s })
    }
  }
  ...
  const handleOpenWaste = async () => {
    ...
    try {
      const [res1, res2] = await Promise.all([
        fetchIngredients({ destination: "bar" }),
        fetchIngredients({ destination: "both" }),
      ])
      const all    = [...(res1.data.ingredients || []), ...(res2.data.ingredients || [])]
      const unique = all.filter((v, i, a) => a.findIndex(x => x._id === v._id) === i)
  ...
  const preparing = allCards.filter(c => c.status === "cooking")
  const ready     = allCards.filter(c => c.status === "ready")

  const columns = [
    {
      key: "pending",  label: "Incoming",       count: pending.length,   cards: pending,
      accent: "#3b82f6", accentMid: "#dbeafe",
      headerBg: "bg-blue-50 border-blue-200",     headerText: "text-blue-700",   headerDot: "bg-blue-500",
      countBg: "bg-blue-500 text-white",          chipBg: "bg-blue-100 text-blue-700",   emptyIcon: "🍹",
    },
    {
      key: "cooking",  label: "Preparing",       count: preparing.length, cards: preparing,
      accent: "#8b5cf6", accentMid: "#ede9fe",
      headerBg: "bg-violet-50 border-violet-200", headerText: "text-violet-700", headerDot: "bg-violet-500",
      countBg: "bg-violet-500 text-white",        chipBg: "bg-violet-100 text-violet-700", emptyIcon: "🍺",
    },
    {
      key: "ready",    label: "Ready to Serve",  count: ready.length,     cards: ready,
      accent: "#14b8a6", accentMid: "#ccfbf1",
      headerBg: "bg-teal-50 border-teal-200",     headerText: "text-teal-700",   headerDot: "bg-teal-500",
      countBg: "bg-teal-500 text-white",          chipBg: "bg-teal-100 text-teal-700",   emptyIcon: "✓",
    },
  ]
    ...
    <div className="min-h-screen bg-zinc-100 flex flex-col">
      ...
      <div className="bg-white border-b border-zinc-200 px-6 h-16 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-500 to-violet-600 flex items-center justify-center text-xl shadow-md shadow-violet-200">
            🍹
          </div>
          <div>
        ...
        <div className="flex items-center gap-2">
          {[
            { label: "Incoming",  count: pending.length,   cls: "bg-blue-50 border-blue-200 text-blue-600",       dot: "bg-blue-500"   },
            { label: "Preparing", count: preparing.length, cls: "bg-violet-50 border-violet-200 text-violet-600", dot: "bg-violet-500" },
            { label: "Ready",     count: ready.length,     cls: "bg-teal-50 border-teal-200 text-teal-700",       dot: "bg-teal-500"   },
          ].map(s => (
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleOpenWaste}
            className="text-xs font-semibold px-3 py-1.5 border border-violet-200 rounded-lg text-violet-600 hover:bg-violet-50 transition-all flex items-center gap-1.5">
      ...
      {loading ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          <div className="w-9 h-9 border-[3px] border-zinc-200 border-t-blue-500 rounded-full animate-spin" />
              ...
              {col.cards.length === 0 ? (
                ...
                col.cards
                  ...
                  .map(card => {
                    ...
                    const isPreparing = preparingItems.has(cardKey)
                    ...
                    // Compute per-item timer display data (preparing column only)
                    ...
                    return (
                      <div key={cardKey}
                        className={`bg-white rounded-2xl border overflow-hidden transition-all duration-300
                          ${col.key === "ready"
                            ? nearEnd ? "border-red-300 shadow-red-100 shadow-md" : "border-teal-300 shadow-teal-100 shadow-md"
                        ...
                        {col.key === "ready" && countdown !== null && (
                          <div className="h-1 bg-zinc-100">
                            <div
                              className={`h-full transition-all duration-1000 ${nearEnd ? "bg-red-400" : "bg-teal-400"}`}
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        )}
                        ...
                        <div className="px-4 pt-3 pb-3 flex items-center justify-between gap-3">
                          ...
                          {col.key === "ready" && countdown !== null ? (
                            <div className={`flex flex-col items-center px-3 py-1.5 rounded-xl flex-shrink-0
                              ${nearEnd ? "bg-red-100 text-red-600 animate-pulse" : "bg-teal-100 text-teal-700"}`}>
                            ...
                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-bold uppercase tracking-wide flex-shrink-0 ${col.chipBg}`}>
                              {col.key === "pending" ? "NEW" : col.key === "cooking" ? "Mixing 🍹" : "Ready ✓"}
                            </span>
                          )}
                        </div>
                        ...
                        {/* Per-item preparing timer (preparing column only) */}
                        {timerUI && (
                          <>
                            <div className="h-px bg-zinc-100 mx-4" />
                        ...
                        <div className="px-4 py-3 flex gap-2">
                          ...
                          {col.key === "pending" && (
                            <button
                              ...
                              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-blue-500 to-violet-600 text-white text-sm font-black tracking-wide shadow-md shadow-violet-200 hover:from-blue-600 hover:to-violet-700 active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
                              {isPreparing
                          ...
                          {col.key === "cooking" && (
                            <button
                              ...
                              disabled={isMarking}
                              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-teal-500 to-teal-600 text-white text-sm font-black tracking-wide shadow-md shadow-teal-200 hover:from-teal-600 hover:to-teal-700 active:scale-[0.98] transition-all disabled:opacity-60 flex items-center justify-center gap-2">
                              {isMarking
      ...
      {/* ESTIMATE PROMPT MODAL — shown when bartender clicks "Start Preparing" */}
      {estimatePrompt && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-xs shadow-2xl border border-zinc-200 overflow-hidden">
            <div className="px-5 pt-5 pb-4 border-b border-zinc-100">
              <h3 className="text-sm font-bold text-zinc-800">How long to prepare?</h3>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button key={opt.label}
                    onClick={() => handleStartPreparing(estimatePrompt, opt.secs)}
                    className="py-3.5 rounded-xl bg-gradient-to-br from-blue-500 to-violet-600 text-white text-sm font-black hover:from-blue-600 hover:to-violet-700 active:scale-[0.96] transition-all shadow-md shadow-violet-100">
                    {opt.label}
                  </button>
                ))}
              </div>
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === "Enter") {
                      ...
                      if (v > 0) handleStartPreparing(estimatePrompt, v * 60)
                    }
                  }}
                  placeholder="Custom (min)"
                  className="flex-1 px-3 py-2.5 rounded-xl border border-zinc-200 text-sm text-center focus:outline-none focus:border-violet-400"
                  autoFocus
                />
                <button
                  disabled={!estimateCustom}
                  onClick={() => { const v = parseInt(estimateCustom); if (v > 0) handleStartPreparing(estimatePrompt, v * 60) }}
                  className="px-4 rounded-xl bg-violet-500 text-white text-sm font-bold disabled:opacity-40 hover:bg-violet-600 transition-all">
                  Set
                </button>
              </div>
              <button
                onClick={() => handleStartPreparing(estimatePrompt, DEFAULT_ESTIMATE_SECS)}
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl border border-zinc-200 overflow-hidden">
            <div className="flex items-start justify-between p-5 border-b border-zinc-100">
              <div>
                ...
                <p className="text-xs text-zinc-400 mt-0.5">Record ingredient waste from the bar</p>
              </div>
            ...
            <form onSubmit={handleWasteSubmit} className="p-5 space-y-4">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-zinc-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 bg-white">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full border border-zinc-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-violet-400 bg-white capitalize">
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button type="submit" disabled={wasteSubmitting}
                  className="flex-1 py-2.5 bg-violet-500 hover:bg-violet-600 disabled:opacity-60 text-white rounded-xl text-sm font-bold transition-colors">
